...
- `project_submissions(project_id, kind, venue, submitted_at, status Submitted|Under review|Revision requested|Accepted|Rejected|Withdrawn|Published, decision_at, manuscript_id, doi, pmid, impact_factor, notes)` — logged from the project's Progress tab; logging auto-advances stage to `submitted`/`published`.
- `research_report(p_from date, p_to date)` RPC → jsonb with KPIs for hub/report.html (printable quarterly report).
- ORCID: `faculty_profiles.orcid` (set in Hub → Directory → Edit profile). sync-pubmed adds `<orcid>[auid]` to the query and treats ORCID-linked papers as confirmed.
- Literature feed: `feed_sources` (journal PubMed terms; admin-editable), `literature_feed` (pmid, title, journal, field Neurosurgery|Neurology|Neuroscience, authors, pub_date, doi, abstract, url, featured, hidden), `feed_sync_log`. Edge fn `sync-feed` runs daily 06:10 UTC via pg_cron → `call_edge('sync-feed')`; 90-day prune weekly. Front-end: `SL.renderFeed(el, {limit, field})` (index.html section + news.html?tab=literature).
...
## v5 additions — beta-test hardening (Aug 2026)
- **Membership gate.** `is_member()` (SECURITY DEFINER) = the caller's profile is `login_approved`, or the role is Admin/IRB. Every internal table (`projects`, `project_*`, `grants`, `deadlines`, `meetings`, `documents*`, `service_requests`, `cv_entries`, `forum_*`, `announcements`, `citi_training`, `audit_log`, `notifications`, all `btr_*`/`tbi_*`/`fnr_*`) carries a RESTRICTIVE policy `members_only` for role `authenticated`. Anonymous/public policies are unchanged. A signed-in but unapproved account can now read only its own `profiles` row — which is what the login page needs to show the "awaiting approval" message. New internal tables must get the same restrictive policy.
- **Sync trigger.** The Hub calls `rpc('admin_run_sync', { p_job, p_body })` — admin-gated, whitelisted to `sync-pubmed | sync-feed | sync-opportunities`, and writes an `audit_log` row. `call_edge` stays available to admins but should not be called from new page code.
- **Grants tightened.** `get_vault_secret`, `get_sync_secret` and the trigger functions (`handle_new_user`, `notify_email_trigger`, `documents_version_trigger`, `log_project_stage`) are no longer executable by `anon`/`authenticated`; `approve_login_request`, `research_report` and `trigger_pubmed_sync` are no longer executable by `anon`.
- **One-time passwords.** `admin-provision` generates a random 12-character password per account (returned once in the response) — never a name-derived pattern. `{ rotate_unused: true, notify?: bool, limit?: n }` re-randomises every account that has never signed in and can email each person their new one-time password.
- **Registry field names.** The `*_patients` tables now use the same column names as the registry pages (e.g. `age_at_diagnosis`, `extent_of_resection`, `tert_promoter`, `ki67_percent`, `radiation_received`, `gcs_field`/`gcs_ed`, `marshall_class`, `rotterdam_score`, `icu_days`, `age_at_surgery`, `updrs_pre`/`updrs_post`, `vas_pre`/`vas_post`, `procedure_type`, `laterality`). Patient entry and the dashboard stats depend on this — keep the page field `key` and the column name identical.
- **Progress is computed, never stored.** Read `project_progress` (a PostgREST computed column over `projects`) or derive it from `project_type_defs.stages`; the legacy `projects.progress` integer is only a fallback for rows with no typed pipeline.
- **Rate charts.** `REG.bar(..., { allowZero: true })` renders a percentage series whose values are all 0 instead of the "no data" placeholder. Use it for every rate/percentage chart.
- **Service request status** is constrained to `new | triaged | in_progress | done | declined` (default `new`); the queues and counters filter on exactly these values.
